...
<div class="body">
  <div class="dingdan">
    ...
    <div class="header_top">
      ...
      <p class="header" style="border-bottom: 2px solid #2b883a;color:#2b883a" @click="shouhuo">待收货</p>
    ...
    <div v-for="(item,index) in NotList" :key="index" class="content">
      ...
      <div class="content_bottom" v-for="(item1,index) in NotiList" :key="index">
        ...
        <div style="width: 70%;" class="right">
          ...
          <h2>共 {{item.goods_num}}件商品 合计：￥<i style="color:#2b883a;">{{item.order_amount}}</i></h2>
          <div class="btn">
            <router-link
              tag="p"
              :to="{path:'/order/LogisticsDetail',query:{pay_type:item.pay_type,order_id:item.order_id}}"
            >查看物流</router-link>
            <p style="color:#2b883a;border:1px solid #2b883a;" @click="xiangqing(item,index)">确认收货</p>
          </div>
        </div>
      </div>
    </div>
