  ...
  <div class="body">
    <div class="dingdan">
      ...
      <div class="header_top">
        <p class="header" style="border-bottom: 2px solid #2b883a;color:#2b883a" @click="quanbu">全部</p>
        <p class="header" @click="fukuan">待付款</p>
        <p class="header" @click="fahuo">待发货</p>
        <p class="header" @click="shouhuo">待收货</p>
        <p class="header" @click="pingjia">待评价</p>
      </div>
      <div style="margin-bottom: 49px;">
        <div v-for="(item,index) in NotList" :key="index" class="content">
          <div class="content_top">
            <p>{{item.add_time | formatDate}}</p>
            <p v-show="item.status===1" style="color:#2b883a">待付款</p>
            <p v-show="item.status===2">待发货</p>
            <p v-show="item.status===3">待收货</p>
            <p v-show="item.status===4">交易成功（待评价）</p>
            <p v-show="item.status===5">订单已取消</p>
            <p v-show="item.status===6">待退款</p>
            <p v-show="item.status===7">已退款</p>
            <p v-show="item.status===8">拒绝退款</p>
          </div>
          <div class="content_bottom" v-for="(item1,index) in NotiList" :key="index">
            ...
            <div style="width: 70%;" class="right">
              ...
              <div class="btn" v-show="item.status===1">
                <p @click="quxiao(item,index)">取消订单</p>
                <p
                  style="color:#2b883a;border:1px solid #2b883a;"
                  @click="fukuan_in(item,index)"
                >去付款</p>
              </div>
              <div class="btn" v-show="item.status===2" @click="xiangxi(item,index)">
                <p style="color:#2b883a;border:1px solid #2b883a;">查看详情</p>
              </div>
              <div class="btn" v-show="item.status===3">
                <router-link
                  tag="p"
                  :to="{path:'/order/LogisticsDetail',query:{pay_type:item.pay_type,order_id:item.order_id}}"
                >查看物流</router-link>
                <p style="color:#6eb92b;border:1px solid #6eb92b;" @click="xiangqing">确认收货</p>
              </div>
              <div class="btn" v-show="item.status===4">
                <router-link
                  tag="p"
                  :to="{path:'/order/OrderDetails',query:{id:item.order_id}}"
                >查看详情</router-link>
                <router-link
                  tag="p"
                  style="color:#7bafe2;border:1px solid #7bafe2;"
...
export default {
  ...
  methods: {
    // 查看详情
    xiangxi(item, index) {
      console.log(item, "458953465");
      localStorage.setItem("order_id", item.order_id);
      this.$router.push("/order/OrderDetails");
    },
    removeByValue: function(NotList, attr, value) {
      //数组，属性，属性值
      var index = 0;
      for (var i in NotList) {
        if (NotList[i][attr] == value) {
          index = i;
          break;
        }
      }
      NotList.splice(index, 1);
    },
    //取消
    quxiao(item, index) {
      ...
      this.$axios
        ...
        .then(res => {
          ...
          if (res.data.status === 200) {
            Toast.success("操作成功");
            setTimeout(function() {
              if (Toast.success("操作成功")) {
                window.location.reload();
                // that.removeByValue(that.NotList,"order_id",item.order_id);
              }
            }, 1000);
    ...
    fukuan_in(item, index) {
      console.log(item, "222");
      localStorage.setItem("order_sn", item.order_sn);
      localStorage.setItem("pay_type", item.pay_type);
      localStorage.setItem("order_id", item.order_id);
      localStorage.setItem("order_amount", item.order_amount);
