<template>
  <div class="AddressView">
    ...
    <!-- <div class="divbox">
       <div class="left" @click="fanghui"><i class="iconfont icon-fanhui"></i></div>
    ...
    <div class="content">
      <div class="address-list" v-for="(item,index) in siteList" :key="index">
        ...
        <div class="operation-bar">
          <router-link :to="'/users/address/EditAddress?address_id='+item.address_id+'quhao='+item.p_cn+item.c_cn+item.d_cn">
            <span class="iconfont icon-bianji1 edit"></span>
          </router-link>
          <span class="iconfont icon-guanbi del-icon" @click="delSite(item,index)"></span>
        </div>
      </div>
      <router-link to="/users/address/AddAddress">
        <div class="add-address">
          <span class="plus-icon">
            <img src="/static/images/user/address/plus-icon.png" />
          </span>
          <span>新建收货地址</span>
        </div>
      </router-link>
...
export default {
  ...
  methods: {
    ...
    fanghui:function(){
      ...
      // this.$router.push("/cart/cartDetail"+id)
    },
    ...
    var url = "address/del_address";
    var params = new URLSearchParams();
    params.append("address_id", item.address_id); // 传给后台的参数值
